...
\newcommand{\school}{the University of Georgia\xspace}
\newcommand{\schoolLong}{Stanford University\xspace}

\newcommand{\profOne}{Prof. In Kee Kim\xspace}
\newcommand{\profTwo}{Dr. Arief Setyanto\xspace}
...
\def\header{%
	\begin{minipage}[]{150pt}
		\hfill Ahmad Naufal Labiib Nabhaan		% Applicant Name
    	\par \hfill 				% Formatting boilerplate
    	nabhaan.ahmad.ln@gmail.com % email
    \end{minipage}
}
\fancyhead[RE,LO]{Statement of Purpose}
...
My objective for pursuing the doctoral program in computer science at \textbf{\school} was to perform research in the fields of \textbf{Computing}, \textbf{Internet of Things}, \textbf{Operating System}, and \textbf{Cloud Computing}. Currently, I am collaborating on a project with \textbf{\profOne} to develop \textbf{a Deep Learning Performance Predictor on Edge Devices using the Deep Learning Performance Measurement with Memory Limitation Scenario}. This research produced a number of hardware performance evaluation scenarios for deep learning applications, as well as a paper that I submitted as first author to \textbf{the IoTDI Conference 2024}.

%% Describe 2-3 past projects that might be relevant to your research interests. (10-12 lines per project)

% Introduce your first experience, which that relatively with your research area

Since high school, computer science has piqued my curiosity. So, after high school, in August 2019, I took the decision to apply at \textbf{the University of AMIKOM Yogyakarta}, one of the top computer science campuses in Indonesia, for an undergraduate computer engineering degree. As an undergraduate student in computer science, I learned plenty of new information while studying at the college. Until one day, in May 2022, I was hired as a research assistant to gather datasets and implement programs on edge devices under the title \textbf{New Normal Adaptation Monitoring Using Multimedia Data Analysis with Deep Learning}, led by \textbf{\profTwo}. That is when I began my research experience.

% PROJECT 1: Mask Detection

My first research experience taught me how to use edge devices like the Jetson Nano and Raspberry Pi to monitor mask detection applications in real-time, utilizing webcams developed in Python and using the YOLOv4 model for object detection. There is an obstacle to running the program, which is that the expectation is high-speed streaming, but the realization is low-speed streaming. The YOLOv4 Tiny model is one of the solutions to the speed streaming constraint, despite the accuracy that must be sacrificed. Furthermore, I assisted in collecting and arranging the image dataset using Roboflow, which could aid in organizing an enormous image dataset that is simpler to handle. I was given the difficult chore of annotating faces in three different scenarios at a particular moment: faces with masks, faces without masks, and faces with improper masks.
